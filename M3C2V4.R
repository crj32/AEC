...
#' comparison against the reference mean. A fast alternative is included that includes a penalty
...
#' @param iters Numerical value: how many Monte Carlo iterations to perform (default: 100, recommended: 5-100)
...
#' @param clusteralg String: dictates which inner clustering algorithm to use
...
#' @examples
#' res <- M3C(mydata)
M3C <- function(mydata, cores = 1, iters = 50, maxK = 10,
                des = NULL, ref_method = c('reverse-pca', 'chol'), repsref = 100, repsreal = 100,
                clusteralg = c('pam', 'km', 'spectral', 'hc'), distance = 'euclidean', pacx1 = 0.1, 
                pacx2 = 0.9, seed=123, objective='entropy', removeplots = FALSE,
                silent = FALSE, fsize = 18, method = 1, lambda = 0.1){
  ...
  if (montecarlo == TRUE){
    ...
    if (removeplots == FALSE){ # we are doing the plots
      ...
      # RCSI
      px <- ggplot(data=real, aes(x=K, y=RCSI)) + geom_line(colour = "slateblue", size = 2) + 
        geom_point(colour = "slateblue", size = 3) +
        theme_bw() +
        ...
        geom_errorbar(aes(ymin=RCSI-qnorm(0.975)*RCSI_SE, ymax=RCSI+qnorm(0.975)*RCSI_SE), 
                      width=.1, colour = "slateblue",size=1.5)
      # pval score
  ...
  if (objective == 'entropy' & method == 2){
    ...
    real$PCSI <- log(real$ENTROPY)+0.1*real$K
    pz <- PCSI_plot(real,fsize=fsize,maxK=maxK)
    if (removeplots == FALSE){
      print(pz)
    }
    plots[[3]] <- pz
  }else if (objective == 'PAC' & method == 2){
    real$PCSI <- log(real$PAC_SCORE)+0.1*real$K
    pz2 <- PCSI_plot(real,fsize=fsize,maxK=maxK)
    if (removeplots == FALSE){
      print(pz2)
    }
    plots[[4]] <- pz2
...
M3Creal <- function( d=NULL, # function for real data
  ...
  pac_res <- CDF(ml, x1=x1, x2=x2, removeplots=removeplots, fsize=fsize, 
...
M3Cref <- function( d=NULL, # function for reference data
  ...
  pac_res <- CDF(ml, x1=x1, x2=x2, removeplots=TRUE, fsize=18, method=1,
...
CDF=function(ml,breaks=100,x1=x1,x2=x2,
  ...
  if (returnplots == TRUE){
    p2 <- ggplot2::ggplot(data=PAC_res_df, ggplot2::aes(x=K, y=PAC_SCORE)) + ggplot2::geom_line(colour = ccol, size = 2) + ggplot2::geom_point(colour = "black", size = 3) +
      ggplot2::theme_bw() + 
      ggplot2::theme(axis.text.y = ggplot2::element_text(size = fsize, colour = 'black'),
                     axis.text.x = ggplot2::element_text(size = fsize, colour = 'black'),
                     axis.title.x = ggplot2::element_text(size = fsize),
...
PCSI_plot <- function(real,fsize=fsize,maxK=maxK){
  p3 <- ggplot2::ggplot(data=real, ggplot2::aes(x=K, y=PCSI)) + ggplot2::geom_line(colour = "tomato", size = 2) + ggplot2::geom_point(colour = "black", size = 3) +
    ggplot2::theme_bw() + 
    ggplot2::theme(axis.text.y = ggplot2::element_text(size = fsize, colour = 'black'),
                   axis.text.x = ggplot2::element_text(size = fsize, colour = 'black'),
                   axis.title.x = ggplot2::element_text(size = fsize),
